...
const {
    ...
    Logger,
    sendTxUsingJito,
    sendTransactionsAsBundleUsingJito

} = require("@solworks/soltoolkit-sdk");
...
class PumpFunSDK {
    ...
    async createAndBuy(creator, mint, createTokenMetadata, buyAmountSol, slippageBasisPoints = 500n, priorityFees, tipFee = 0, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        ...
        if (buyAmountSol > 0) {
            const gas = this.gasInstruction(priorityFees, tipFee, creator.publicKey)
        ...
        let createResults = await (0, util_1.sendTx)(this.connection, newTx, creator.publicKey, [creator, mint], priorityFees, tipFee, commitment, finality);
        return createResults;
    }
    gasInstruction(priorityFees, tipFee, sender) {
        const newTx = new web3_js_1.Transaction()
        const tip = web3_js_1.SystemProgram.transfer({
            fromPubkey: sender,
            toPubkey: new web3_js_1.PublicKey("DttWaMuVvTiduZRnguLF7jNxTgiMBZ1hyAumKUiL2KRL"),
            lamports: tipFee
        })
    ...
    async createAndBuyAndSnipe(creator, mint, createTokenMetadata, buyAmountSol, snipePrivateKeys, snipeAmounts, slippageBasisPoints = 500n, priorityFees, tipFee = 0, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        let tokenMetadata = await this.createTokenMetadata(createTokenMetadata);
        let createTx = await this.getCreateInstructions(creator.publicKey, createTokenMetadata.name, createTokenMetadata.symbol, tokenMetadata.metadataUri, mint);
        let createAndBuyTx = new web3_js_1.Transaction();
        const globalAccount = await this.getGlobalAccount(commitment);
        let txsSigned = [];
        if (buyAmountSol > 0) {
            const gas = this.gasInstruction(priorityFees, tipFee, creator.publicKey)
            ...
            const buyAmountWithSlippage = (0, util_1.calculateWithSlippageBuy)(buyAmountSol, slippageBasisPoints);
            const buyTx = await this.getBuyInstructions(creator.publicKey, mint.publicKey, globalAccount.feeRecipient, buyAmount, buyAmountWithSlippage);
            createAndBuyTx.add(gas);
            createAndBuyTx.add(createTx);
            createAndBuyTx.add(buyTx);
        }

        let versionedCreateAndBuyTx = await util_1.buildVersionedTx(this.connection, creator.publicKey, createAndBuyTx, util_1.DEFAULT_COMMITMENT);
        versionedCreateAndBuyTx.sign([creator, mint])
        txsSigned.push(versionedCreateAndBuyTx);

        //buy tx
        for (let _ = 0; _ < snipePrivateKeys.length; _++) {
            const sender = snipePrivateKeys[_];
            const amount = snipeAmounts[_];
            const gas = this.gasInstruction(priorityFees, tipFee, sender.publicKey)
            const buyAmount = globalAccount.getInitialBuyPrice(amount);
            const buyAmountWithSlippage = (0, util_1.calculateWithSlippageBuy)(amount, BigInt(10000));
            const buyInstruction = await this.getBuyInstructions(sender.publicKey, mint.publicKey, globalAccount.feeRecipient, buyAmount, buyAmountWithSlippage);
            buyInstruction.add(gas)
            let versionedTx = await util_1.buildVersionedTx(this.connection, sender.publicKey, buyInstruction, util_1.DEFAULT_COMMITMENT)
            versionedTx.sign([sender])
            txsSigned.push(versionedTx);
        }
        const buyResults = await sendTransactionsAsBundleUsingJito({
            signedTxs: txsSigned,
            region: 'mainnet',
        });
        console.log(`bundle id ${buyResults}`);
        return buyResults;
    }

    async bundleBuy(keypairs, mint, buyAmountSols, slippageBasisPoints = 500n, priorityFees, tipFee = 0, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        let txsSigned = [];
        for (let _ = 0; _ < keypairs.length; _++) {
            const sender = keypairs[_];
            const amount = buyAmountSols[_];
            const newTx = new web3_js_1.Transaction()
            if (priorityFees) {
                const modifyComputeUnits = web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({
                    units: priorityFees.unitLimit,
            ...
            const buyInstruction = await this.getBuyInstructionsBySolAmount(
                sender.publicKey,
                mint,
                amount,
                slippageBasisPoints,
            ...
            newTx.add(buyInstruction)
            let versionedTx = await util_1.buildVersionedTx(this.connection, sender.publicKey, newTx, util_1.DEFAULT_COMMITMENT)
            versionedTx.sign([sender])
            txsSigned.push(versionedTx);
        }
        const buyResults = await sendTransactionsAsBundleUsingJito({
    ...
    async buy(buyer, mint, buyAmountSol, slippageBasisPoints = 500n, priorityFees, tipFee = 0, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        let buyTx = await this.getBuyInstructionsBySolAmount(buyer.publicKey, mint, buyAmountSol, slippageBasisPoints, commitment);
        let buyResults = await (0, util_1.sendTx)(this.connection, buyTx, buyer.publicKey, [buyer], priorityFees, tipFee, commitment, finality);
        return buyResults;
    }

    async sell(seller, mint, sellTokenAmount, priorityFees, tipFee = 0, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        const newTx = new web3_js_1.Transaction();
        const gas = this.gasInstruction(priorityFees, tipFee, seller.publicKey);
        let sellTx = await this.getSellInstructionsByTokenAmount(seller.publicKey, mint, sellTokenAmount, commitment);
        newTx.add(gas);
        newTx.add(sellTx)
        let sellResults = await (0, util_1.sendTx)(this.connection, newTx, seller.publicKey, [seller], commitment, finality);
        return sellResults;
    }
    ...
    //sell
    async getSellInstructionsByTokenAmount(seller, mint, sellTokenAmount, commitment = util_1.DEFAULT_COMMITMENT) {
        let bondingCurveAccount = await this.getBondingCurveAccount(mint, commitment);
        if (!bondingCurveAccount) {
            throw new Error(`Bonding curve account not found: ${mint.toBase58()}`);
        }
        ...
        return await this.getSellInstructions(seller, mint, globalAccount.feeRecipient, sellTokenAmount);
    }
    async getSellInstructions(seller, mint, feeRecipient, amount, minSolOutput) {
        ...
        let transaction = new web3_js_1.Transaction();
        transaction.add(await this.program.methods
            .sell(new bn_js_1.BN(amount.toString()), new bn_js_1.BN(0))
            .accounts({
                feeRecipient: feeRecipient,
                mint: mint,
                associatedBondingCurve: associatedBondingCurve,
